...
## Decision
- **The `mlock`'d head `BufferPool` is fixed:** `--memory` bytes on the explicit/automatic local NUMA
  node, plus `--sub-memory` bytes on every other online node when explicitly requested. The latter
  requires `--numa NODE`. Head allocation is local-first, then foreign, and never grows.
...
## Consequences
- ➕ Hard, predictable footprint; the `mlock`'d pools never swap; local heads stay local until that
...
- ✅ Fixed, `mlock`'d, per-NUMA-region pool backs the RAM-head cache, with strict local-first
